...
def get_im_gt_name_dict(datasets, flag='valid'):
    ...
    for i in range(len(datasets)):
        ...
        tmp_im_list, tmp_gt_list,tmp_eg_list = [], [],[]
        ...
        if(datasets[i]["eg_dir"]==""):
            print('-eg-', datasets[i]["name"], datasets[i]["eg_dir"], ': ', 'No Ground Truth Found')
            tmp_eg_list = []
        else:
            tmp_eg_list = [datasets[i]["eg_dir"]+os.sep+x.split(os.sep)[-1].split(datasets[i]["im_ext"])[0]+datasets[i]["eg_ext"] for x in tmp_im_list]
            print('-eg-', datasets[i]["name"],datasets[i]["eg_dir"], ': ',len(tmp_eg_list))
...
class RandomHFlip(object):
    ...
    def __call__(self,sample):
        imidx, image, label,edge_label, shape,im_name =  sample['imidx'], sample['image'], sample['label'],sample['edge_label'], sample['shape'],sample['im_name']

        # random horizontal flip
        if random.random() >= self.prob:
            image = torch.flip(image,dims=[2])
            label = torch.flip(label,dims=[2])
            edge_label = torch.flip(edge_label,dims=[2])

        return {'imidx':imidx,'image':image, 'label':label,'edge_label':edge_label , 'shape':shape,'im_name':im_name}
...
class Resize(object):
    ...
    def __call__(self,sample):
        ...
        edge_label = torch.squeeze(F.interpolate(torch.unsqueeze(edge_label,0),self.size,mode='bilinear'),dim=0)

        return {'imidx':imidx,'image':image, 'label':label,'edge_label':edge_label, 'shape':torch.tensor(self.size),'im_name':im_name}
...
class RandomCrop(object):
    ...
    def __call__(self,sample):
        ...
        edge_label = edge_label[:,top:top+new_h,left:left+new_w]
...
class Normalize(object):
    ...
    def __call__(self,sample):
        ...
        imidx, image, label, shape,edge_label,im_name=  sample['imidx'], sample['image'], sample['label'], sample['shape'],sample['edge_label'],sample['im_name']
...
class LargeScaleJitter(object):
    ...
    def __call__(self, sample):
        imidx, image, label, image_size,edge_label,im_name =  sample['imidx'], sample['image'], sample['label'], sample['shape'],sample['edge_label'],sample['im_name']
        #resize keep ratio
        ...
        scaled_edge_label = torch.squeeze(F.interpolate(torch.unsqueeze(edge_label,0),scaled_size.tolist(),mode='bilinear'),dim=0)
        ...
        scaled_edge_label = scaled_edge_label[:,crop_y1:crop_y2, crop_x1:crop_x2]
        ...
        edge_label = F.pad(scaled_edge_label, [0,padding_w, 0,padding_h],value=0)

        return {'imidx':imidx,'image':image, 'label':label,"edge_label":edge_label,'shape':torch.tensor(image.shape[-2:]),'im_name':im_name}
...
class OnlineDataset(Dataset):
    def __init__(self, name_im_gt_list, transform=None, eval_ori_resolution=False):
        ...
        for i in range(0,len(name_im_gt_list)):
            ...
            eg_path_list.extend(name_im_gt_list[i]["eg_path"])
            ...
            eg_ext_list.extend([name_im_gt_list[i]["eg_ext"] for x in name_im_gt_list[i]["eg_path"]])
        ...
        self.dataset["eg_path"] = eg_path_list
        self.dataset["ori_eg_path"] = deepcopy(eg_path_list)
        self.dataset["eg_ext"] = eg_ext_list
    ...
    def __getitem__(self, idx):
        ...
        eg_path = self.dataset["eg_path"][idx]
        im_name = self.dataset["im_name"][idx]
        im = io.imread(im_path)
        gt = io.imread(gt_path)
        eg = io.imread(eg_path)
        if len(gt.shape) > 2:
            gt = gt[:, :, 0]
        if len(eg.shape) > 2:
            eg = eg[:, :, 0]
        ...
        eg = torch.unsqueeze(torch.tensor(eg, dtype=torch.float32),0)
        ...
        if self.eval_ori_resolution:
            ...
            sample['ori_eg_path'] = self.dataset["eg_path"][idx]
